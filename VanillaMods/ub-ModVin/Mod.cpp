...
UB_EXPORT void Initialization(ModData* data)
{
	...
	{
		std::function<bool(const std::vector<std::string>&, const uint64_t&)>* execution = new std::function<bool(const std::vector<std::string>&, const uint64_t&)>();
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			return data->Variable->SetReal(idx, std::stoull(args[0]), data->Object->CreateObject(new RuntimeObject()));
		};
	...
	}
	{
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			return data->Variable->SetReal(idx, std::stoull(args[1]), data->Byte->AllocateBytes(std::stod(args[0])));
		};
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			...
			return data->Variable->SetReal(idx, std::stoull(args[1]), *bytes);
		};
	...
	}
	{
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			return data->Variable->SetBool(idx, std::stoull(args[0]), sf::Mouse::isButtonPressed(sf::Mouse::Left));
		};
	...
	}
	{
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			return data->Variable->SetBool(idx, std::stoull(args[0]), sf::Mouse::isButtonPressed(sf::Mouse::Right));
		};
	...
	}
	{
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			return data->Variable->SetBool(idx, std::stoull(args[0]), sf::Mouse::isButtonPressed(sf::Mouse::Middle));
		};
	...
	}
	{
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			return data->Variable->SetReal(idx, std::stoull(args[0]), sf::Mouse::getPosition(*data->Runtime->Window).x);
		};
	...
	}
	{
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			return data->Variable->SetReal(idx, std::stoull(args[0]), sf::Mouse::getPosition(*data->Runtime->Window).y);
		};
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			...
			return data->Variable->SetReal(idx, std::stoull(args[0]), value);
		};
	...
	}
	{
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			return data->Variable->SetBool(idx, std::stoull(args[1]), sf::Keyboard::isKeyPressed((sf::Keyboard::Key)(uint64_t)floor(std::stod(args[0]))));
		};
	...
	}
	{
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			const double* gotValue = data->Variable->GetReal(idx, std::stoull(args[0]));

			if (gotValue == nullptr)
				return false;

			return data->Variable->SetReal(idx, std::stoull(args[0]), *gotValue + std::stod(args[1]));
		};
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			...
			return data->Variable->SetReal(idx, std::stoull(args[0]), (uint64_t)floor(*gotValue) % (uint64_t)floor(std::stod(args[1])));
		};
		RegBlock* block = data->Registry->CreateBlock("vin_operations_%=", "vin_operations", execution, {
			{BlockArgumentType::REAL, BlockArgumentVariableModeRestriction::ONLY_VAR_KEEP, BlockArgumentVariableMode::VAR, "var"},
			{BlockArgumentType::TEXT, BlockArgumentVariableModeRestriction::NONE, BlockArgumentVariableMode::RAW, "%="},
	...
	}
	{
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			return data->Variable->SetReal(idx, std::stoull(args[0]), std::stod(args[1]));
		};
	...
	}
	{
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			return data->Variable->SetString(idx, std::stoull(args[0]), args[1]);
		};
	...
	}
	{
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			return data->Variable->SetBool(idx, std::stoull(args[0]), args[1] == "1");
		};
	...
	}
	{
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			return data->Variable->SetBool(idx, std::stoull(args[0]), args[1] == "0");
		};
		RegBlock* block = data->Registry->CreateBlock("vin_operations_!=_bool", "vin_operations", execution, {
			{BlockArgumentType::BOOL, BlockArgumentVariableModeRestriction::ONLY_VAR_KEEP, BlockArgumentVariableMode::VAR, "var"},
			{BlockArgumentType::TEXT, BlockArgumentVariableModeRestriction::NONE, BlockArgumentVariableMode::RAW, "!="},
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			...
			return data->Variable->SetReal(idx, std::stoull(args[0]), floor(*gotValue));
		};
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			...
			return data->Variable->SetReal(idx, std::stoull(args[0]), round(*gotValue));
		};
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			...
			return data->Variable->SetReal(idx, std::stoull(args[0]), ceil(*gotValue));
		};
	...
	}
	{
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			return data->Variable->SetReal(idx, std::stoull(args[0]), ((double)(rand() % 1000)) / 1000.0);
		};
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			...
			return data->Variable->SetReal(idx, std::stoull(args[0]), abs(*gotValue));
		};
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			...
			return data->Variable->SetReal(idx, std::stoull(args[0]), pow(*gotValue, std::stod(args[1])));
		};
	...
	}
	{
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			return data->Variable->SetBool(idx, std::stoull(args[2]), std::stod(args[0]) == std::stod(args[1]));
		};
		RegBlock* block = data->Registry->CreateBlock("vin_operations_==_real", "vin_operations", execution, {
			{BlockArgumentType::REAL, BlockArgumentVariableModeRestriction::NONE, BlockArgumentVariableMode::VAR, "var"},
			{BlockArgumentType::TEXT, BlockArgumentVariableModeRestriction::NONE, BlockArgumentVariableMode::RAW, "=="},
	...
	}
	{
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			return data->Variable->SetBool(idx, std::stoull(args[2]), args[0] == args[1]);
		};
		RegBlock* block = data->Registry->CreateBlock("vin_operations_==_string", "vin_operations", execution, {
	...
	}
	{
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			return data->Variable->SetString(idx, std::stoull(args[1]), args[0].c_str());
		};
	...
	}
	{
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			return data->Variable->SetReal(idx, std::stoull(args[1]), std::stod(args[0]));
		};
	...
	}
	{
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			return data->Variable->SetBool(idx, std::stoull(args[1]), args[0] == "1");
		};
	...
	}
	{
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			return data->Variable->SetReal(idx, std::stoull(args[1]), args[0].length());
		};
	...
	}
	{
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			return data->Variable->SetString(idx, std::stoull(args[3]), args[0].substr(std::stoull(args[1]), std::stoull(args[2])));
		};
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			...
			if (!data->Variable->SetReal(idx, std::stoull(args[2]), threadId))
			{
	...
	}
	{
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			return data->Variable->StackReal(idx, std::stoull(args[0]));
		};
	...
	}
	{
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			return data->Variable->StackString(idx, std::stoull(args[0]));
		};
	...
	}
	{
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			return data->Variable->StackBool(idx, std::stoull(args[0]));
		};
	...
	}
	{
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			data->Variable->HeapVariable(idx, std::stoull(args[0]));
			return true;
		};
	...
	}
	{
		...
		*execution = [data](const std::vector<std::string>& args, const uint64_t& idx)
		{
			return data->Variable->FreeVariable(idx, std::stoull(args[0]));
		};
