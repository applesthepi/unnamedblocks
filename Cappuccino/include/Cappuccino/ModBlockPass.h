...
public:
	...
	CAP_DLL uint64_t* GetReal(const uint64_t& idx);
...
private:
	...
	uint64_t* (ModBlockPass::* m_getReal)(const uint64_t& idx);
	uint64_t* GetRealDebug(const uint64_t& idx);
	uint64_t* GetRealRelease(const uint64_t& idx);
