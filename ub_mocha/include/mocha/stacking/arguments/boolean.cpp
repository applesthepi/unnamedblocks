...
rhr::stack::argument::boolean::boolean(
	const espresso::color& color, std::function<void()>* function_collection_update, glm::vec<2, i32>* plane_offset)
	...
	, m_decor_right(std::make_shared<rhr::render::object::object>(true))
{
	m_mouse_button = [&, color](glm::vec<2, i32> position, f32 scroll, rhr::handler::input::mouse_operation operation, rhr::handler::input::mouse_button button)
	{
		...
		{
			if (m_mode == espresso::mod::block::block::argument::variable_mode::RAW)
				set_mode(espresso::mod::block::block::argument::variable_mode::VAR);
			else if (m_mode == espresso::mod::block::block::argument::variable_mode::VAR)
	...
	m_text->set_color_primary(espresso::color::text_primary_color);
	m_text->set_color_secondary(color);
	m_text->set_padding(padding);
	m_text->set_mouse_button(m_mouse_button);
}

espresso::mod::block::block::argument::type rhr::stack::argument::boolean::get_type()
{
	return espresso::mod::block::block::argument::type::BOOL;
}
...
void rhr::stack::argument::boolean::ui_update_buffers()
{
	...
	espresso::color use_color = m_block_color;
	if (m_mode == espresso::mod::block::block::argument::variable_mode::VAR)
		use_color = espresso::color().from_u8({100, 0, 40, 255});

	rhr::render::vertex vertices[6];

	vertices[0] = rhr::render::vertex(
		{static_cast<f32>(ARG_BOOL_DECORE_WIDTH), 0.0f, 0}, use_color.get_normalized(), {0.0f, 0.0f});
	vertices[1] = rhr::render::vertex(
		{static_cast<f32>(0.0f), 0.0f /*static_cast<f32>(rhr::stack::block::height_content / 2)*/, 0},
		use_color.get_normalized(),
		{1.0f, 0.0f});
	vertices[2] = rhr::render::vertex(
		{static_cast<f32>(ARG_BOOL_DECORE_WIDTH), static_cast<f32>(rhr::stack::block::height_content), 0},
		use_color.get_normalized(),
		{1.0f, 1.0f});

	vertices[3] = rhr::render::vertex(
		{static_cast<f32>(fullWidth - ARG_BOOL_DECORE_WIDTH), 0.0f, 0}, use_color.get_normalized(), {0.0f, 0.0f});
	vertices[4] = rhr::render::vertex(
		{static_cast<f32>(fullWidth), static_cast<f32>(rhr::stack::block::height_content /* / 2*/), 0},
		use_color.get_normalized(),
		{1.0f, 0.0f});
	vertices[5] = rhr::render::vertex(
		{static_cast<f32>(fullWidth - ARG_BOOL_DECORE_WIDTH), static_cast<f32>(rhr::stack::block::height_content), 0},
		use_color.get_normalized(),
		{1.0f, 1.0f});
...
rhr::stack::argument::argument::padding_style rhr::stack::argument::boolean::get_padding_style()
{
	return rhr::stack::argument::argument::padding_style::NONE;
}
...
void rhr::stack::argument::boolean::on_set_mode(espresso::mod::block::block::argument::variable_mode mode)
{
	if (m_mode == espresso::mod::block::block::argument::variable_mode::RAW)
		m_text->set_color_secondary(m_block_color);
	else if (m_mode == espresso::mod::block::block::argument::variable_mode::VAR)
		m_text->set_color_secondary(espresso::color().from_u8({100, 0, 40, 255}));
}
