...
rhr::stack::argument::text::text(
	...
	, m_text(std::make_shared<rhr::render::object::text>(
		  rhr::registry::char_texture::texture_type::BOLD_NORMAL,
		  ...
		  plane_offset))
{
	...
	m_text->set_color_primary(espresso::color::black);
	m_text->set_color_secondary(color);
	m_text->set_padding(0);
	m_text->enable_background(false);
}

espresso::mod::block::block::argument::type rhr::stack::argument::text::get_type()
{
...
{
	//	latte::logger::info("arg text width: " + std::to_string(m_text->get_size_local().x));
...
void rhr::stack::argument::text::ui_transform_update(i_ui::transform_update_spec transform_update_spec)
{
