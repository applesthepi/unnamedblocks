...
rhr::render::object::button::~button() { InputHandler::UnregisterMouseCallback(mouse_update_caller); }
...
void rhr::render::object::button::mouse_update(glm::vec<2, i32> position, f32 scroll, MouseOperation operation)
{
	...
	if (operation == MouseOperation::Click)
	{
		...
		if (position.x >= button_position.x && position.x < button_position.x + get_size_local().x && position.y >= button_position.y
			&& position.y < button_position.y + get_size_local().y)
		{
...
void rhr::render::object::button::post_color_update() { m_background->set_color(m_color_secondary); }
