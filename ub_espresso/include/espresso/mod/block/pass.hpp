...
public:
	...
	{
	public:
		initializer(std::vector<f64*>& t_real, std::vector<bool*>& t_bool, std::vector<std::string*>& t_string);
		...
		///
		espresso::mod::block::data** block_data;

		///
		u64 function_total_count;

		///
		u64* function_call_count;

		///
		std::vector<f64*>& template_real;
		std::vector<bool*>& template_bool;
		std::vector<std::string*>& template_string;
	};
	...
	///
	void set_data(espresso::mod::block::data** data);
...
private:
	...
	// user data
	///
	espresso::mod::block::data** m_data;
