...
void rhr::render::object::rectangle::ui_update_buffers()
{
	...
	if (m_border_enabled)
	{
		...
		if (m_overhang_horizontal == cardinal::local_horizontal::LEFT)
		{
			...
			v_left0 = rhr::render::vertex({-1.0f, 0.0, 0.0}, espresso::color::black.get_normalized(), {0.0f, 0.0f});
			v_left1 = rhr::render::vertex({0.0, 0.0, 0.0}, espresso::color::black.get_normalized(), {1.0f, 0.0f});
			v_left2 = rhr::render::vertex({0.0, use_size.y, 0.0}, espresso::color::black.get_normalized(), {1.0f, 1.0f});
			v_left3 = rhr::render::vertex({-1.0f, use_size.y, 0.0}, espresso::color::black.get_normalized(), {0.0f, 1.0f});

			v_right0 =
				rhr::render::vertex({use_size.x - 1.0f, 0.0, 0.0}, espresso::color::black.get_normalized(), {0.0f, 0.0f});
			v_right1 = rhr::render::vertex({use_size.x, 0.0, 0.0}, espresso::color::black.get_normalized(), {1.0f, 0.0f});
			v_right2 =
				rhr::render::vertex({use_size.x, use_size.y, 0.0}, espresso::color::black.get_normalized(), {1.0f, 1.0f});
			v_right3 = rhr::render::vertex(
				{use_size.x - 1.0f, use_size.y, 0.0}, espresso::color::black.get_normalized(), {0.0f, 1.0f});
		}
		else if (m_overhang_horizontal == cardinal::local_horizontal::RIGHT)
		{
			// overhang border to the right

			v_left0 = rhr::render::vertex({0.0, 0.0, 0.0}, espresso::color::black.get_normalized(), {0.0f, 0.0f});
			v_left1 = rhr::render::vertex({1.0f, 0.0, 0.0}, espresso::color::black.get_normalized(), {1.0f, 0.0f});
			v_left2 = rhr::render::vertex({1.0f, use_size.y, 0.0}, espresso::color::black.get_normalized(), {1.0f, 1.0f});
			v_left3 = rhr::render::vertex({0.0, use_size.y, 0.0}, espresso::color::black.get_normalized(), {0.0f, 1.0f});

			v_right0 = rhr::render::vertex({use_size.x, 0.0, 0.0}, espresso::color::black.get_normalized(), {0.0f, 0.0f});
			v_right1 =
				rhr::render::vertex({use_size.x + 1.0f, 0.0, 0.0}, espresso::color::black.get_normalized(), {1.0f, 0.0f});
			v_right2 = rhr::render::vertex(
				{use_size.x + 1.0f, use_size.y, 0.0}, espresso::color::black.get_normalized(), {1.0f, 1.0f});
			v_right3 =
				rhr::render::vertex({use_size.x, use_size.y, 0.0}, espresso::color::black.get_normalized(), {0.0f, 1.0f});
		}
		...
		if (m_overhang_vertical == cardinal::local_vertical::UP)
		{
			...
			v_up0 = rhr::render::vertex({0.0, -1.0, 0.0}, espresso::color::black.get_normalized(), {0.0f, 0.0f});
			v_up1 = rhr::render::vertex({use_size.x, -1.0, 0.0}, espresso::color::black.get_normalized(), {1.0f, 0.0f});
			v_up2 = rhr::render::vertex({use_size.x, 0.0, 0.0}, espresso::color::black.get_normalized(), {1.0f, 1.0f});
			v_up3 = rhr::render::vertex({0.0, 0.0, 0.0}, espresso::color::black.get_normalized(), {0.0f, 1.0f});

			v_down0 =
				rhr::render::vertex({0.0, use_size.y - 1.0f, 0.0}, espresso::color::black.get_normalized(), {0.0f, 0.0f});
			v_down1 = rhr::render::vertex(
				{use_size.x, use_size.y - 1.0f, 0.0}, espresso::color::black.get_normalized(), {1.0f, 0.0f});
			v_down2 =
				rhr::render::vertex({use_size.x, use_size.y, 0.0}, espresso::color::black.get_normalized(), {1.0f, 1.0f});
			v_down3 = rhr::render::vertex({0.0, use_size.y, 0.0}, espresso::color::black.get_normalized(), {0.0f, 1.0f});
		}
		else if (m_overhang_vertical == cardinal::local_vertical::DOWN)
		{
			// overhang border down

			v_up0 = rhr::render::vertex({0.0, 0.0, 0.0}, espresso::color::black.get_normalized(), {0.0f, 0.0f});
			v_up1 = rhr::render::vertex({use_size.x, 0.0, 0.0}, espresso::color::black.get_normalized(), {1.0f, 0.0f});
			v_up2 = rhr::render::vertex({use_size.x, 1.0, 0.0}, espresso::color::black.get_normalized(), {1.0f, 1.0f});
			v_up3 = rhr::render::vertex({0.0, 1.0, 0.0}, espresso::color::black.get_normalized(), {0.0f, 1.0f});

			v_down0 = rhr::render::vertex({0.0, use_size.y, 0.0}, espresso::color::black.get_normalized(), {0.0f, 0.0f});
			v_down1 =
				rhr::render::vertex({use_size.x, use_size.y, 0.0}, espresso::color::black.get_normalized(), {1.0f, 0.0f});
			v_down2 = rhr::render::vertex(
				{use_size.x, use_size.y + 1.0f, 0.0}, espresso::color::black.get_normalized(), {1.0f, 1.0f});
			v_down3 =
				rhr::render::vertex({0.0, use_size.y + 1.0f, 0.0}, espresso::color::black.get_normalized(), {0.0f, 1.0f});
		}
